...
export default function Card(props: { item: ICard }) {
  const {
    item: { avatar, country, image, feedback, name, rating },
  } = props;
  const ratingItems = Array(5)
    .fill(0)
    .map((i, index) => (
      <FontAwesomeIcon
        icon={faStar}
      ...
      <div className="hover:bg-[#ffff]  hover:scale-110  duration-500 rounded-md p-4 text-text      pb-4   flex flex-col align-middle items-center justify-center  h-fit bg-[#E3DFFD] border-colorid7e810469 ">
        <div className=" w-full flex flex-row justify-center pb-2 mb-2">
          ...
          <div className="flex flex-col pl-4 h-full justify-around  pt-4  ">
            ...
            <p className="text-sm">
              <span
                className={`fi mr-2 fi-${
                  flagDictionary[country as keyof Object]
                }`}
              />
              {country}
            </p>
            <div className="flex flex-row">{ratingItems}</div>
          </div>
        </div>
        {image && (
          <img src={image} alt="avatar" className="h-24 py-2 rounded-md" />
        )}
        <p className="text-sm text-center break-all ">{feedback}</p>
      </div>
    </div>
  );
}
